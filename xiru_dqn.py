...
class DQN:
    ...
    def __init__(self, args, n_actions=4, hidden=1024,
        ...
        self.target_reward = tf.placeholder(shape=[None, 1], dtype=tf.float32)
        ...
        self.policy_input = tf.concat([self.target_reward, self.d], axis=1)
        self.policy_l1 = tf.layers.dense(inputs = self.policy_input, units = hidden, activation=tf.nn.relu,
                                     kernel_initializer=tf.variance_scaling_initializer(scale=2), name="policy_1" )
        self.policy_l2 = tf.layers.dense(inputs = self.policy_l1, units = hidden, activation=tf.nn.relu, 
                                     kernel_initializer=tf.variance_scaling_initializer(scale=2), name="policy_2" )
        self.policy_l3 = tf.layers.dense(inputs = self.policy_l2, units = hidden, activation=tf.nn.relu, 
                                     kernel_initializer=tf.variance_scaling_initializer(scale=2), name="policy_3" )
        self.policy_logits = tf.layers.dense(
            inputs=self.policy_l3, units=self.n_actions,
            kernel_initializer=tf.variance_scaling_initializer(scale=2), name='policy_4')
        ...
        self.policy_action_logits =  tf.reduce_sum(self.target_action_prob(tf.multiply(self.policy_action_prob, self.one_hot_action)), axis=1)
...
def train(name="dqn_xiru", priority=True):
    ...
    if priority:
        my_replay_memory = PriorityBuffer.PrioritizedReplayBuffer(MEMORY_SIZE, alpha=args.alpha, batch_size=args.batch_size)  #
    else:
        my_replay_memory = PriorityBuffer.ReplayBuffer(MEMORY_SIZE, batch_size=args.batch_size)  #
    ...
    if os.path.exists(args.expert_dir + args.expert_file) and not args.delete_expert:
