...
class toy_maze:
    def __init__(self, file, expert_dir='./', grid=10, final_reward=2.0, reward=2.0, cost=-0.05, obstacle_cost=0,
                 level=15, expert=True,
    ...
    def reset(self, eval=False, expert=False):
        ...
        for i in range(self.grid):
            for j in range(self.grid):
                new_current_state[i, j] = (np.abs(self.current_state_x - i) + np.abs(
                    self.current_state_y - j)) / self.grid - 1
        for i in range(4):
    ...
    def step(self, action):
        ...
        if self.board[x, y, self.agent_history_length] == -0.5 or (
                self.current_state_x == x and self.current_state_y == y):
            pass
        ...
        self.new_board[:, :, 0:self.agent_history_length - 1] = np.copy(
            self.new_board[:, :, 1:self.agent_history_length])
    ...
    while count < min(grid - 1, 10):
        ...
        while l <= min(length, grid // 3) and condition:
            ...
            if dist_to_r == 0:
                condition = False
            dist_to_o = min([np.abs(state[0] - x[0]) + np.abs(state[1] - x[1]) +
                             np.abs(np.abs(state[0] - x[0]) - np.abs(state[1] - x[1])) for x in obstacles[:-(l)]] or [
                                99])
            if dist_to_o <= 2:
                condition = False
