...
def train(priority=True, agent='model', grid=10, seed=0):
    ...
    with tf.variable_scope(agent):
        ...
        if args.env_id == 'maze':
            ...
            env_test = toy_maze_grid('/home/yutonyan/Q-model/test_mazes',level=3,expert=False)
        else:
            final_reward = 1
            env = toy_env(grid, final_reward)
        ...
        while frame_number < MAX_FRAMES:
            ...
            if last_eval > 100:
                last_eval=0
                test_eps_reward,test_eps_reward_t = eval(args,env_test,env,action_getter,sess,MAIN_DQN)
                print(test_eps_reward)
...
def eval(args,env_test,env,action_getter,sess,MAIN_DQN):
    ...
    for level in range(6):
        ...
        episode_length = 0
        while episode_length < 60 and not terminal:
    ...
    return eps_reward,test_eps_reward
