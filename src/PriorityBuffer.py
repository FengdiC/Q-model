...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def expert_decay(self,frame_num,eps_initial = 1,eps_annealing_frames=20000000,replay_memory_start_size=150000):
        slope = -eps_initial / eps_annealing_frames
        intercept = eps_initial - slope * replay_memory_start_size
        if frame_num < replay_memory_start_size:
            eps = eps_initial
        elif frame_num >= replay_memory_start_size and frame_num < replay_memory_start_size + eps_annealing_frames:
            eps = slope * frame_num + intercept
        elif frame_num >= replay_memory_start_size + eps_annealing_frames:
            eps = 0
        return eps

    def update_priorities(self, idxes, priorities, expert_idxes, frame_num, expert_weight=0.8):
        ...
        expert_priority_modifier = min(self._max_priority, 1 + self.expert_decay(frame_num,expert_weight))
        count = 0
        ...
        for idx, priority in zip(idxes, priorities):
            ...
            if expert_idxes[count] == 1:
                new_priority = priority * expert_priority_modifier
            else:
