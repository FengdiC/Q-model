...
def train(priority=True, agent='model', grid=10, seed=0):
    ...
    with tf.variable_scope(agent):
        ...
        if args.env_id == 'maze':
            env = toy_maze('mazes',expert_dir=args.expert_dir,level=20)
...
def eval(args,env_test,env_val,env,action_getter,sess,MAIN_DQN, TARGET_DQN, frame_num, rand_moves=4):
    episode_length=0
    eps_reward=0
    env.restart()
    plot=True
...
def generate_max_q_value_map(args, sess, env, main_dqn, target_dqn, frame_num):
...
def plot_state(state,index, grid=10):
